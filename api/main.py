...
from schemas import Patient, Appointment, Treatment, Billing, ChatRequest, hospital_catalog, ChatResponse
...
from chatbot import answer_query, load_db, build_db
import logging
...
def get_hospital_catalog( ):

    try:

        df = pd.read_csv(
            "csv_files/hospital_dataset.csv"
        )
...
def startup():
    ...
    try:
        db = load_db()
        log.info("Vector DB loaded")
    except Exception as e:
        log.error(f"DB load failed: {e}")
        db = build_db()
        log.info("Vector DB created")
...
def chatbot(req: ChatRequest):    
    ...
    result = answer_query(db, req.query)
